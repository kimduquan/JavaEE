...
docker run --name vllm --runtime nvidia --gpus all -v ~/.cache/huggingface:/root/.cache/huggingface --env "HUGGING_FACE_HUB_TOKEN=" --env "VLLM_API_KEY=EMPTY" -p 8000:8000 --ipc=host vllm/vllm-openai:latest --model neuralmagic/granite-3.1-8b-instruct-quantized.w4a16 --gpu-memory-utilization 0.87 --max_model_len 15936 --enable-auto-tool-choice --tool-call-parser granite 