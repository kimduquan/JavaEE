...
CSI_REGISTRAR_IMAGE_TAG=v2.17.0
...
../../registry.sh registry.k8s.io "sig-storage/csi-node-driver-registrar:$CSI_REGISTRAR_IMAGE_TAG"
...
helm --kubeconfig="${EPF_CLUSTER_KUBE_CONFIG}" upgrade --install --create-namespace --namespace rook-ceph --wait --timeout 40m rook-ceph rook/rook-ceph -f values.yaml \
	...
	--set "csi.registrar.repository=$EPF_CLUSTER_IMAGE_REGISTRY/sig-storage/csi-node-driver-registrar" \
	--set "csi.registrar.tag=$CSI_REGISTRAR_IMAGE_TAG" \
...
kubectl --kubeconfig=${EPF_CLUSTER_KUBE_CONFIG} wait -n rook-ceph -l app=rook-ceph.cephfs.csi.ceph.com-ctrlplugin pod --for condition=Ready --timeout=1200s

: > export-env.sh

./export-external-cluster.sh > export-env.sh
...
export KUBECONFIG=
kubectl --kubeconfig=${EPF_CLUSTER_KUBE_CONFIG} wait -n rook-ceph cephcluster rook-ceph --for condition=Connected=True --timeout=1200s
